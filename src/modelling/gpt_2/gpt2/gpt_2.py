    ...
    def sample_batch():
        ...
        if batch_size > 1:
            sampled_batch = np.asarray([
                np.pad(
                    v, [0, batch_len - len(v)], 'constant', constant_values=63)
...
def generate_to_file(sess,
    ...
    generate(sess, False, truncate, destination_path, sample_delim, prefix,
             model_name, seed, nsamples, batch_size, length, temperature, top_k,
             run_name, include_prefix)
...
def copy_checkpoint_to_gdrive(checkpoint_folder=os.path.join(
        'checkpoint', 'run1')):
...
def copy_checkpoint_from_gdrive(checkpoint_folder=os.path.join(
        'checkpoint', 'run1')):
    """Copies the checkpoint folder from a mounted Google Drive."""
    is_mounted()

    shutil.copytree("/content/drive/My Drive/" + checkpoint_folder,
                    checkpoint_folder)
...
def cmd():
    ...
    parser = argparse.ArgumentParser(
        description=
        "Easily retrain OpenAI's GPT-2 text-generating model on new texts. (https://github.com/minimaxir/gpt-2-simple)"
    )
    ...
    parser.add_argument(
        '--mode',
        help=
        'Mode for using the CLI (either "finetune" or "generate") [Required]',
        nargs='?')
    parser.add_argument(
        '--run_name',
    ...
    parser.add_argument('--dataset',
                        help="[finetune] Path to the source text.",
                        nargs='?',
                        default=None)
    parser.add_argument(
        '--steps',
    ...
    parser.add_argument(
        '--restore_from',
        help=
        "[finetune] Whether to load model 'fresh' or from 'latest' checkpoint.",
        nargs='?',
        default='latest')
    ...
    parser.add_argument('--include_prefix',
    ...
    if args.mode == 'finetune':
        ...
        cmd_finetune(dataset=args.dataset,
    ...
    if args.mode == "generate":
        cmd_generate(nfiles=args.nfiles,
...
def cmd_generate(nfiles, nsamples, folder, length, temperature, batch_size,
                 prefix, truncate, include_prefix, sample_delim):
