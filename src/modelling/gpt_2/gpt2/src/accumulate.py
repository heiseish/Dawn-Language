...
class AccumulatingOptimizer(object):
    ...
    def __init__(self, opt, var_list):
        ...
        self.accum_vars = {
            tv: tf.Variable(tf.zeros_like(tv.initialized_value()),
                            trainable=False) for tv in var_list
        }
    ...
    def reset(self):
        ...
        updates.append(
            self.total_loss.assign(tf.zeros(shape=[], dtype=tf.float32)))
        updates.append(
            self.count_loss.assign(tf.zeros(shape=[], dtype=tf.float32)))
