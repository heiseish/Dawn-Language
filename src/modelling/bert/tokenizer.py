''' Tokenize a sentence into known otokens'''
import unicodedata
from typing import Optional, Dict, List, NoReturn, Tuple

from .bert import TOKEN_CLS, TOKEN_SEP, TOKEN_UNK


class Tokenizer(object):
    ''' Tokenizer for sentence
    '''

    def __init__(self,
                 token_dict: Dict[str, str],
                 token_cls: str = TOKEN_CLS,
                 token_sep: str = TOKEN_SEP,
                 token_unk: str = TOKEN_UNK,
                 pad_index: int = 0,
                 cased: bool = False):
        '''Initialize tokenizer.
        Args
            token_dict: A dict maps tokens to indices.
            token_cls: The token represents classification.
            token_sep: The token represents separator.
            token_unk: The token represents unknown token.
            pad_index: The index to pad.
            cased: Whether to keep the case.
        '''
        self._token_dict = token_dict
        self._token_cls = token_cls
        self._token_sep = token_sep
        self._token_unk = token_unk
        self._pad_index = pad_index
        self._cased = cased

    @staticmethod
    def _truncate(first_tokens: List[str],
                  second_tokens: Optional[List[str]] = None,
                  max_len: Optional[int] = None) -> NoReturn:
        ''' Truncate the sentence so that the total number of token does not exceed max_len - 3 special tokens
        Args:
            first_tokens (List[str]): List of  first tokens
            second_tokens (Optional[List[str]]): List of second tokens
            max_len (Optional[int]): max len
        '''
        if max_len is None:
            return

        if second_tokens is not None:
            while True:
                total_len = len(first_tokens) + len(second_tokens)
                if total_len <= max_len - 3:  # 3 for [CLS] .. tokens_a .. [SEP] .. tokens_b [SEP]
                    break
                if len(first_tokens) > len(second_tokens):
                    first_tokens.pop()
                else:
                    second_tokens.pop()
        else:
            del first_tokens[max_len - 2:]  # 2 for [CLS] .. tokens .. [SEP]

    def _pack(self,
              first_tokens: List[str],
              second_tokens: Optional[List[str]] = None
             ) -> Tuple[List[str], int, int]:
        ''' Pack the first and second tokens as well as the lengths into tuples
        Args:
            first_tokens (List[str]): List of  first tokens
            second_tokens (Optional[List[str]]): List of second tokens
        Returns:
            Tuple containing packed first + [second] token, length of first packed tokens, lengths of second packed token
        '''
        first_packed_tokens = [self._token_cls
                              ] + first_tokens + [self._token_sep]
        if second_tokens is not None:
            second_packed_tokens = second_tokens + [self._token_sep]
            return first_packed_tokens + second_packed_tokens, len(
                first_packed_tokens), len(second_packed_tokens)
        else:
            return first_packed_tokens, len(first_packed_tokens), 0

    def _convert_tokens_to_ids(self, tokens=List[str]) -> List[int]:
        ''' Convert the token into numeric id
        Args:
            tokens (List[str]): list of string tokens
        Returns:
            List of numeric id
        '''
        unk_id = self._token_dict.get(self._token_unk)
        return [self._token_dict.get(token, unk_id) for token in tokens]

    def tokenize(self, first: str, second: Optional[str] = None) -> List[str]:
        ''' Tokenize the text into tokens
        Args:
            first (str): first text
            second (str): second text
        Returns:
            list of tokens from the texts
        '''
        first_tokens = self._tokenize(first)
        second_tokens = self._tokenize(second) if second is not None else None
        tokens, _, _ = self._pack(first_tokens, second_tokens)
        return tokens

    def encode(self,
               first: str,
               second: Optional[str] = None,
               max_len: Optional[int] = None) -> Tuple[List[int], List[int]]:
        ''' Encode the texts into token ids and sections id
        Args:
            first (str): first text
            second (str): second text
        Returns:
            Tuple containing token ids and segment ids
        '''
        first_tokens = self._tokenize(first)
        second_tokens = self._tokenize(second) if second is not None else None
        self._truncate(first_tokens, second_tokens, max_len)
        tokens, first_len, second_len = self._pack(first_tokens, second_tokens)

        token_ids = self._convert_tokens_to_ids(tokens)
        segment_ids = [0] * first_len + [1] * second_len

        if max_len is not None:
            pad_len = max_len - first_len - second_len
            token_ids += [self._pad_index] * pad_len
            segment_ids += [0] * pad_len

        return token_ids, segment_ids

    def _tokenize(self, text: str) -> List[str]:
        ''' Tokenize a string into list of tokens
        Args:
            text (str): text to be tokenized
        Returns:
            list of tokens
        '''
        if not self._cased:
            text = unicodedata.normalize('NFD', text)
            text = ''.join(
                [ch for ch in text if unicodedata.category(ch) != 'Mn'])
            text = text.lower()
        spaced = ''
        for ch in text:
            if self._is_punctuation(ch) or self._is_cjk_character(ch):
                spaced += ' ' + ch + ' '
            elif self._is_space(ch):
                spaced += ' '
            elif ord(ch) == 0 or ord(ch) == 0xfffd or self._is_control(ch):
                continue
            else:
                spaced += ch
        tokens = []
        for word in spaced.strip().split():
            tokens += self._word_piece_tokenize(word)
        return tokens

    def _word_piece_tokenize(self, word: str) -> List[str]:
        ''' Tokenize a word
        Args:
            word (str): word to be tokenized
        Returns:
            List of tokinzed character
        '''
        if word in self._token_dict:
            return [word]
        tokens = []
        start, stop = 0, 0
        while start < len(word):
            stop = len(word)
            while stop > start:
                sub = word[start:stop]
                if start > 0:
                    sub = '##' + sub
                if sub in self._token_dict:
                    break
                stop -= 1
            if start == stop:
                stop += 1
            tokens.append(sub)
            start = stop
        return tokens

    @staticmethod
    def _is_punctuation(ch: str) -> bool:
        ''' Check whether a character is a punctuation
        Args:
            ch: character to be checked
        Returns:
            True if the character is a punctuation, False otherwise
        '''
        code = ord(ch)
        return 33 <= code <= 47 or \
            58 <= code <= 64 or \
            91 <= code <= 96 or \
            123 <= code <= 126 or \
            unicodedata.category(ch).startswith('P')

    @staticmethod
    def _is_cjk_character(ch: str) -> bool:
        ''' Check whether a character is a CJK [https://en.wikipedia.org/wiki/CJK_characters] character
        Args:
            ch (str) : character to be checked
        Returns:
            True if the cahracter is CJK character, False otherwise
        '''
        code = ord(ch)
        return 0x4E00 <= code <= 0x9FFF or \
            0x3400 <= code <= 0x4DBF or \
            0x20000 <= code <= 0x2A6DF or \
            0x2A700 <= code <= 0x2B73F or \
            0x2B740 <= code <= 0x2B81F or \
            0x2B820 <= code <= 0x2CEAF or \
            0xF900 <= code <= 0xFAFF or \
            0x2F800 <= code <= 0x2FA1F

    @staticmethod
    def _is_space(ch: str) -> bool:
        ''' Check whether a character is a break-type character
        Args:
            ch (str): character to be checked
        Returns:
            True if the character is a break-type character, False otherwise
        '''
        return ch == ' ' or ch == '\n' or ch == '\r' or ch == '\t' or \
            unicodedata.category(ch) == 'Zs'

    @staticmethod
    def _is_control(ch: str) -> bool:
        ''' Check whether a character is a control-type character
        Args:
            ch(str): character to be checked
        Returns:
            True if the character is a control-type character, False otherwise
        '''
        return unicodedata.category(ch).startswith('C')
