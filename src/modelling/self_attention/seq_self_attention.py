...
class SeqSelfAttention(keras.layers.Layer):
    ...
    def __init__(self,
                 ...
                 attention_type: str = ATTENTION_TYPE_ADD,
                 ...
                 attention_regularizer_weight: float = 0.0,
                 **kwargs):
    ...
    def get_config(self) -> Dict[str, str]:
        config = {
            ...
            'kernel_initializer':
            keras.regularizers.serialize(self.kernel_initializer),
            'bias_initializer':
            keras.regularizers.serialize(self.bias_initializer),
            'kernel_regularizer':
            ...
            keras.activations.serialize(self.attention_activation),
    ...
    def _build_multiplicative_attention(self, input_shape=Tuple[int, ...]
                                       ) -> NoReturn:
        feature_dim = input_shape[2]
    ...
    def _call_additive_emission(self, inputs: tf.Tensor) -> tf.Tensor:
    ...
    def _call_multiplicative_emission(self, inputs: tf.Tensor) -> tf.Tensor:
    ...
    def _attention_regularizer(self, attention: tf.Tensor) -> tf.Tensor:
        ...
        return self.attention_regularizer_weight * K.sum(
            K.square(
                K.batch_dot(attention, K.permute_dimensions(
                    attention, (0, 2, 1))) - eye)) / batch_size
